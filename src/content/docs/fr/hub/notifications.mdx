...
Watchflare envoie alertes et résolutions par un ou plusieurs **canaux**, via [Shoutrrr](https://shoutrrr.nickfedor.com/). Chaque canal est une URL Shoutrrr vers une seule destination (webhook Discord, Incoming Webhook Slack, boîte SMTP, bot Telegram…). Tout se gère dans **Settings > Notifications**.
...
Chaque canal s'abonne à une ou plusieurs **catégories**, via les cases du tiroir Add ou Edit. Il ne reçoit une notification que s'il est abonné à sa catégorie : supervision d'un côté, sécurité du compte de l'autre.
...
<Callout type="info" title="Remarque">
  Le [SMTP dédié](/fr/hub/smtp/) a le même sélecteur : **Alerts** pour les alertes, **Transactional** pour le compte et la sécurité. Cet expéditeur n'est pas un canal `smtp://` ajouté ici — celui-là se comporte comme n'importe quel canal Shoutrrr.
</Callout>
...
|-------|---------------|------------------|
| **New login** | Connexion réussie, y compris après une étape 2FA | Heure, IP, appareil (user agent) |
...
<Callout type="info" title="Remarque">
  Ces notifications partent en arrière-plan, sans garantie. Elles ne bloquent jamais l'action, et un échec d'envoi ne la fait pas échouer non plus : si rien n'est abonné à **Transactional**, l'événement a lieu quand même, sans message.
</Callout>

<Callout type="warn" title="Attention">
  L'IP d'une notification de connexion est celle vue par le Hub. Derrière un reverse proxy, elle n'est exacte que si le proxy pose les en-têtes forwarded, et que le Hub est configuré pour leur faire confiance. Voir [Reverse proxy](/fr/hub/reverse-proxy/).
</Callout>
...
Topic public ntfy.sh sans authentification : `ntfy://ntfy.sh/{topic}`.
...
Le bouton **Test** du tiroir Add ou Edit envoie une notification unique avec l'URL du formulaire, **sans rien enregistrer**. Validez l'URL avant **Create** ou **Save**.
...
La table `webhook_endpoints` des anciennes versions disparaît à la mise à niveau. L'URL était native (URL Discord complète) ; Shoutrrr attend une forme compacte (`discord://TOKEN@WEBHOOK_ID`). Réajoutez vos destinations dans **Settings > Notifications** après la mise à niveau.
