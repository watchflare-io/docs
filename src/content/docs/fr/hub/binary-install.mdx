...
Le Hub est un binaire Linux tout prêt (x86_64 et arm64). Ici vous l'installez en service systemd, TimescaleDB dans Docker.
...
Le Hub a besoin de TimescaleDB pour le stockage. Le plus simple : le Compose fourni.
...
# TLS_MODE=auto         # défaut : génère une CA auto-signée pour le gRPC
# COOKIE_SECURE=true    # à activer si le Hub est derrière un reverse proxy HTTPS
...
Le binaire Hub existe aussi pour macOS (arm64/amd64). L'installation en service macOS n'est pas documentée. En local, lancez le binaire à la main, ou passez par [Docker Compose](/fr/hub/deploy-docker/).
