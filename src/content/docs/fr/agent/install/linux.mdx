...
Remplacez `YOUR_HUB_IP` par l'IP ou le nom d'hôte du Hub, et `wf_reg_YOUR_TOKEN` par le jeton complet copié dans le tableau de bord.
...
**2. Enrôler quand vous êtes prêt**
...
sudo systemctl enable watchflare-agent     # au démarrage
sudo systemctl disable watchflare-agent    # plus au démarrage
...
| L'hôte reste **offline** après le démarrage | Horloge décalée | NTP doit tourner : le Hub refuse un horodatage hors fenêtre de ±5 min |
