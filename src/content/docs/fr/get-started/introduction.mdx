...
description: "Supervision open source et auto-hébergée. Des agents légers remontent métriques et inventaire de paquets vers le Hub en gRPC/TLS, avec tableau de bord temps réel et alertes."
---
...
Le **Hub** est le serveur central. Vous le déployez une fois, en [Docker Compose](/fr/hub/deploy-docker/) ou en [binaire Linux](/fr/hub/binary-install/). Il reçoit les données des agents, les enregistre dans une base time-series et sert le tableau de bord web. Un seul binaire Go, le frontend est embarqué.
...
<figure class="doc__diagram">
  <img
    ...
    alt="Des agents sur des hôtes Linux (systemd) et macOS (Homebrew) remontent les métriques vers le Hub en gRPC, TLS 1.3 et HMAC. Le Hub, un binaire Go unique qui sert un tableau de bord SvelteKit embarqué, stocke les données dans PostgreSQL/TimescaleDB en SQL sur le port TCP 5432."
    width="2163"
    height="1128"
    loading="lazy"
    decoding="async"
...
- **Alertes.** Seuils sur CPU, mémoire, disque, charge et température, plus un hôte qui passe hors ligne. Envoi par e-mail ou vers l'un des 26+ canaux [Shoutrrr](https://shoutrrr.nickfedor.com/) (Discord, Slack, Telegram, Matrix, Ntfy, Gotify, HTTP, etc.).
- **Journal d'écriture anticipée (WAL).** Si le Hub est injoignable, l'agent met les métriques de côté en local et les renvoie à la reconnexion. Une coupure courte ne fait pas perdre de points : 10 Mo par défaut (`wal_max_size_mb`).
