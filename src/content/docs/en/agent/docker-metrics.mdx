...
If the agent is already installed and registered, enable container metrics in three steps:
...
On macOS (Docker Desktop or Colima), the agent runs as the user who started the Homebrew service. That user already has access to the Docker socket, so no group change is needed.
