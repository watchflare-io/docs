...
The agent is configured via a TOML file written at registration. You rarely need to edit it manually, since most fields are set by the `register` command. The file is reloaded on service restart.
...
<Callout type="warn">
  `agent.conf` contains `agent_key`, so treat it like a password. The file is mode `0640` (root:watchflare on Linux). Never expose its contents.
</Callout>
...
|-------|------|---------|-------------|
| `server_host` | string | None | Hub hostname or IP address |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `ca_cert_file` | string | None | Path to the Hub's CA certificate, written at registration |
