...
Store the dump somewhere outside the Docker host. The `pki_data` volume is equally important, since agents are registered against the CA it contains. Without the original CA, re-registered agents must be re-enrolled from scratch.
