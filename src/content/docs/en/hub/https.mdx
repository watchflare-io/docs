...
The Hub serves its dashboard over HTTP on port `8080`. To expose it over HTTPS, put it behind a reverse proxy that handles TLS termination. After doing so, update two environment variables so that session cookies are marked `Secure` correctly.
