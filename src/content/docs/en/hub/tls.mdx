...
- The server certificate CN does not need to match the Hub's hostname. It must match the `server_name` value in `agent.conf` (default: `watchflare`). Using `HostSNI("*")` on your reverse proxy avoids any CN mismatch. See [Reverse proxy](/hub/reverse-proxy/).
