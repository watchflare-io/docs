...
<Callout type="danger">
  Keep these values secret. Back them up outside Docker volumes. Changing `JWT_SECRET` invalidates all active user sessions and disables 2FA for any users who had it enabled (their TOTP secrets can no longer be decrypted — they must re-enroll). Changing `NOTIFICATION_ENCRYPTION_KEY` makes any saved email credentials unreadable — you will need to re-enter them.
</Callout>
...
- [Notification channels](/hub/notifications/) - send alerts to Discord, Slack, Telegram, Matrix, Ntfy, Gotify, SMTP, and 20+ other services via Shoutrrr
