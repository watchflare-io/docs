...
description: "Configure Traefik, Nginx, or Caddy in front of the Watchflare Hub. Covers HTTP reverse proxy for the dashboard and TCP passthrough for the gRPC port."
---
...
During the TLS handshake, the agent sends an SNI equal to `server_name` in `agent.conf`, which defaults to `watchflare` (the certificate CN generated by the Hub). A rule like `HostSNI("watchflare")` would need to match this value exactly.
...
The Hub's dashboard is a single-page app: a cold page load fetches dozens of static assets from `/_app/immutable/*` in parallel, more so on heavier pages. If your proxy enforces a request rate limit, the burst must be high enough to absorb a full page load. A burst that is too low returns `429` for some of those assets, which the browser reports as `NS_ERROR_CORRUPTED_CONTENT` and a failed page load (the dashboard then shows an "Internal Error" page).

<Callout type="warn">
  Rate limiting immutable static assets by request count rarely adds value and easily breaks the dashboard. Use a generous burst (a few hundred), or scope the rate limit to API paths only.
</Callout>

The values shown below are only examples. Size the burst to your own page weight and client count. There is no required number.

For Traefik, this is the `burst` value on the `rateLimit` middleware:
...
For Nginx, use `limit_req` with a `burst` large enough for a full page load (add `nodelay` so the burst is served immediately, not queued):
...
Caddy has no built-in rate limiting, so there is nothing to tune by default. If you add rate limiting through a plugin, size it generously or scope it to API paths only, following the same principle.
