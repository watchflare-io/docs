...
For the full list of alert types and configuration options, see the [Hub alert reference](/hub/alerts/).
...
1. A notification is sent when the incident **opens**.
2. A second notification is sent when the incident **resolves** (metric returns below the threshold, or the host comes back online).
...
Notifications are delivered to one or more notification channels (Discord, Slack, Telegram, Matrix, Ntfy, Gotify, SMTP, generic HTTP, and more via [Shoutrrr](https://shoutrrr.nickfedor.com/)). Channels are configured in **Settings > Notifications**.
...
**Notification channels** deliver the same alerts to any of the Shoutrrr destinations listed above. See [Notification channels](/hub/notifications/).
