---
title: "Package inventory & update detection"
description: "Track every installed package across your fleet with Watchflare. Detects outdated packages and flags pending security updates across ~30 package manager integrations."
---
...
<Callout type="info">
  The **Packages** tab shows "No packages found" for the first 60 seconds after agent startup. This is normal, as the first scan has not completed yet.
</Callout>
...
Each collector self-disables if its tool is not installed on the host, so only collectors with something to report are active.
