...
Watchflare can collect per-container metrics from Docker and Podman hosts: CPU usage, memory, and network I/O for each running container. This feature is opt-in.
...
When the agent itself runs inside a Docker container (rather than on the host), container metrics are not available, since the agent cannot reach the Docker socket from inside the container. Install the agent directly on the host OS to enable container metric collection.
