...
|----------|-----------|-------------|
| `POSTGRES_PASSWORD` | None | Database password. The binary falls back to `watchflare_dev` if unset, so always set this in production. Docker Compose enforces it via `:?`. |
...
| `TLS_CERT_FILE` | None | Server certificate path (`custom` mode only) |
| `TLS_KEY_FILE` | None | Server private key path (`custom` mode only) |
| `TLS_CA_FILE` | None | CA certificate path, sent to agents at registration (`custom` mode only) |
