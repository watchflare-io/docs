---
title: "Watchflare: Self-Hosted Host Monitoring"
description: "Open-source, self-hosted host monitoring: agents report metrics to the Hub over gRPC/TLS. Learn what the dashboard shows and which platforms are supported."
---
...
**Hub** — the central server. Deploy it once with [Docker Compose](/hub/deploy-docker/) or as a [native Linux binary](/hub/binary-install/). It receives data from agents, stores it in a time-series database, and serves the web dashboard. The Hub is a single binary with the frontend embedded.
...
- **Alerts** — threshold-based alerts on CPU, memory, and disk, plus host offline notifications delivered to any of the 26+ services supported by [Shoutrrr](https://shoutrrr.nickfedor.com/) (Discord, Slack, Telegram, Matrix, Ntfy, Gotify, SMTP, generic HTTP, and more).
